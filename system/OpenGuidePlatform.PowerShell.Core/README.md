...
Fonts are never installed or substituted. `fontSources` in any `pdf.yaml` maps a font name to where to get it (a URL or a short note); `Test-GuidePdfFonts -WorkspaceRoot ... -Policy ...` lists every font the site's generated PDFs need, whether it is installed, the settings file that chose it and its source. `New-GuidePdf` checks all fonts first and reports every missing one with the same details. For right-to-left languages the platform switches babel to `bidi=bidi-r` (the default XeTeX mode loses colour changes) and the `rtl-latin` filter marks Latin runs and Latin-only paragraphs as English so their word and digit order survive; cover values are marked the same way.
